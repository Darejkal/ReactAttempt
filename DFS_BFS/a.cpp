...
bool directed_isExistEdge(int a, int b) {
    string da=data[a],db=data[b];
    sort(da.begin()+1,da.end());
    sort(db.begin(),db.end());
    int i=1,j=0;
    while(i<STR_SIZE&&j<STR_SIZE){
        // cout<<da[i]<<" "<<db[j]<<"\n";
        if(da[i]==db[j]){
            i++;
        }
...
class Node {
   public:
    ...
    int current;
    int distance;
    Node(int parent, int current, int distance) {
        this->parent = parent;
...
class Heap {
   ...
   public:
    ...
    void insert(Node val) {
        a.push_back(val);
        heapifyParent(a.size() - 1);
    }
    void heapify(int i) {
        if (i <= a.size()) return;
        int m = i;
        if (2 * i + 1 < a.size() && a[2 * i + 1] < a[m]) {
            m = 2 * i + 1;
        }
        if (2 * i + 2 < a.size() && a[2 * i + 2] < a[m]) {
            m = 2 * i + 2;
        }
        if (m != i) {
            swap(a[m], a[i]);
    ...
    void decreaseKeyTo(int i, int val) {
        if (val > a[i].current) {
            throw invalid_argument("DecreaseKeyInvalidArgument: val must be smaller than current key");
        } else {
            a[i].current = val;
            heapifyParent(i);
        }
...
// Câu 1 a làm nhầm thành tìm số chu trình:
int countCycleSubgraph() {
    int n = data.size();
    int count = 0;
    vector<int> traveled(n, 0);
    while (true) {
        int b = find(traveled.begin(), traveled.end(), 0) - traveled.begin();
        if (b == n)
            return count;
        queue<int> q;
        q.push(b);
        while (!q.empty()) {
            int x = q.front();
            q.pop();
            // all the possible words that match with this word data[x] all will have been traveled
            // so mark this word as negative because it will be no long of use in later run.
            traveled[x] = -1;
            for (int i = b + 1; i < n; i++) {
                if (isConnected(x, i)) {
                    if (traveled[i] == 1) {
                        count++;
                    } else if (traveled[i] == 0) {
                        traveled[i] = 1;
                        q.push(i);
                    }
                }
...
vector<string> shortestPath() {
    string us, vs;
    cin >> us >> vs;
    int u = find(data.begin(), data.end(), us) - data.begin(), v = find(data.begin(), data.end(), vs) - data.begin();
    if (u == data.size() || v == data.size()) {
        throw invalid_argument("InvalidShortestPathArgument: String entered does not exist in data");
    }
    vector<int> parents(data.size(), -1);
    parents[u] = u;
    queue<int> q;
    q.push(u);
    while (!q.empty()) {
        ...
        for (int i = 0; i < data.size(); i++) {
            if (parents[i] < 0 && isConnected(i, cur)) {
                if (i == v) {
                    parents[v] = cur;
    ...
    return vector<string>(1, "PathNotFound");
}
// Câu 2 a
int directed_countStronglyConnectedSubgraph() {
    vector<bool> visited(data.size(),0);
    stack<int> s;
    while(true){
        int x=find(visited.begin(),visited.end(),0)-visited.begin();
        if(x==data.size()) break;
        s.push(x);
        for(int i=x;i<data.size();i++){
            if(!visited[i]&&directed_isExistEdge(i,x)){
                visited[i]=1;
                s.push(i);
            }
        }
    ...
    while(!s.empty()){
        ...
        while (!q.empty()) {
            int x = q.front();
            q.pop();
            for (int i = temp + 1; i < data.size(); i++) {
                if (!visited[i]&&directed_isExistEdge(x, i)) {
                    visited[i] = 1;
                    q.push(i);
                }
            }
...
// Câu 2 b
vector<string> directed_othersInConnectedSubgraph(){
    string us;
    cin >> us;
    int u = find(data.begin(), data.end(), us) - data.begin();
    if (u == data.size()) {
        throw invalid_argument("InvalidShortestPathArgument: String entered does not exist in data");
    }
    vector<bool> traveled(data.size(), 0);
    vector<bool> ntraveled(data.size(), 0);
    traveled[u]=1;
    ntraveled[u]=1;
    queue<int> q;
    q.push(u);
    while (!q.empty()) {
        int x = q.front();
        q.pop();
        for (int i = u + 1; i < data.size(); i++) {
            if (!traveled[i]&&directed_isExistEdge(x, i)) {
                    traveled[i] = 1;
                    q.push(i);
                }
            }
    ...
    while (!q.empty()) {
        ...
        q.pop();
        for (int i = u + 1; i < data.size(); i++) {
            if (!ntraveled[i] &&directed_isExistEdge(i,x)) {
                    ntraveled[i] = 1;
                    q.push(i);
                }
            }
...
// Câu 2 c
vector<string> directed_shortestPath() {
    string us, vs;
    cin >> us >> vs;
    int u = find(data.begin(), data.end(), us) - data.begin(), v = find(data.begin(), data.end(), vs) - data.begin();
    if (u == data.size() || v == data.size()) {
    ...
    while (!q.empty()) {
        ...
        q.pop();
        for (int i = 0; i < data.size(); i++) {
            if (parents[i] < 0 && directed_isExistEdge(cur, i)) {
                if (i == v) {
                    parents[v] = cur;
                    vector<string> result;
                    while (v != u) {
...
int main() {
    // Câu 1:
    //  Phần a:
    cout << countConnectedSubgraph() << "\n";
    //  Phần b:
    printArray(shortestPath());
    //Câu 2:
    //  Phần a:
    cout << directed_countStronglyConnectedSubgraph();
    //  Phần b:
    printArray(directed_othersInConnectedSubgraph());
    //  Phần c:
    printArray(directed_shortestPath());
}
